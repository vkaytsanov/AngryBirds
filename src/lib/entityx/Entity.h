 ...
 */
	...
	public:
		 ...
		 * components. Use destroy() to destroy the associated Entity and getComponents.
		 */
		void invalidate();
...
/**
 * A ComponentHandle<C> is a wrapper around an instance of a getComponent.
 *
 * It provides safe access to getComponents. The handle will be invalidated under
 * the following conditions:
 *
 * - If a getComponent is removed from its host entity.
 ...
 */
	...
	public:
		...
		/**
		 * Remove the getComponent from its entity and destroy it.
		 */
		void remove();

		/**
		 * Returns the Entity associated with the getComponent
		 */
		Entity entity();
...
/**
 * Base getComponent class, only used for insertion into collections.
 *
 ...
 *
 * This is a struct to imply that getComponents should be data-only.
 *
 * Usage:
 *
...
/**
 * Emitted when any getComponent is added to an entity.
 */
	template<typename C>
...
/**
 * Emitted when any getComponent is removed from an entity.
 */
	template<typename C>
...
/**
 * Helper class to perform getComponent operations in a typed manner.
 */
...
/**
 * Manages Entity::Id creation and getComponent assignment.
 */
	...
	public:
		...
		public:
			...
			const Iterator end() const {
				return Iterator(manager_, mask_, static_cast<uint32_t>(manager_->capacity()), unpacker_);
			}
		...
		Entity create() {
			...
			event_manager_.post<EntityCreatedEvent>(entity);
			return entity;
		}

		/**
		 * Create a new Entity by copying another. Copy-constructs each getComponent.
		 *
		 * Emits EntityCreatedEvent.
		 */
		...
		void destroy(Entity::Id entity) {
			...
			event_manager_.post<EntityDestroyedEvent>(Entity(this, entity));
		 ...
		 *
		 *     Position &position = em.addComponent<Position>(e, x, y);
		 *
		...
		ComponentHandle<C> addComponent(Entity::Id id, Args&& ... args) {
			...
			// Placement new into the getComponent pool.
			Pool<C>* pool = accomodate_component<C>();
			::new(pool->get(id.index())) C(std::forward<Args>(args) ...);

			// Set the bit for this getComponent.
			entity_component_mask_[id.index()].set(family);

			// Create and return handle.
			ComponentHandle<C> component(this, id);
			event_manager_.post<ComponentAddedEvent<C>>(Entity(this, id), component);
			return component;
		}
		...
		void remove(Entity::Id id) {
			...
			// Find the pool for this getComponent family.
			BasePool* pool = component_pools_[family];
			ComponentHandle<C> component(this, id);
			event_manager_.post<ComponentRemovedEvent<C>>(Entity(this, id), component);

			// Remove getComponent bit.
		...
		/**
		 * Check if an Entity has a getComponent.
		 */
		template<typename C>
		bool has_component(Entity::Id id) const {
			assert_valid(id);
			size_t family = component_family<C>();
			// We don't bother checking the getComponent mask, as we return a nullptr anyway.
		...
		/**
		 * Find Entities that have all of the specified Components and addComponent them
		 ...
		 * for (Entity entity : entity_manager.entities_with_components(position, direction)) {
		 *   // Use position and getComponent here.
		 * }
		 * @endcode
		 */
		template<typename ... Components>
		...
		/**
		 * Unpack getComponents directly into pointers.
		 *
	...
	ComponentHandle<C> Entity::getComponent() const {
		assert(valid());
	...
	inline void ComponentHandle<C, EM>::remove() {
		assert(valid());
		manager_->template removeComponent<C>(id_);
	}
...
namespace std {
	...
	template<>
	struct hash<const entityx::Entity> {
		std::size_t operator()(const entityx::Entity& entity) const {
			return static_cast<std::size_t>(entity.id().index() ^ entity.id().version());
		}
	};
