...
namespace Clara {
    ...
    const unsigned int consoleWidth = 80;
...
#endif
            virtual void set( ConfigT& m_config, std::string const& value ) const = 0;
        ...
        public:
            ...
            void set( ConfigT& m_config, std::string const& value ) const {
                functionObj->set( m_config, value );
            }
    ...
    public:
        ...
        ConfigT parse( std::vector<std::string> const& args ) const {
            ConfigT m_config;
            parseInto( args, m_config );
            return m_config;
        }

        std::vector<Parser::Token> parseInto( std::vector<std::string> const& args, ConfigT& m_config ) const {
            std::string processName = args.empty() ? std::string() : args[0];
            std::size_t lastSlash = processName.find_last_of( "/\\" );
            if( lastSlash != std::string::npos )
                processName = processName.substr( lastSlash+1 );
            m_boundProcessName.set( m_config, processName );
            std::vector<Parser::Token> tokens;
            Parser parser;
            parser.parseIntoTokens( args, tokens );
            return populate( tokens, m_config );
        }

        std::vector<Parser::Token> populate( std::vector<Parser::Token> const& tokens, ConfigT& m_config ) const {
            validate();
            std::vector<Parser::Token> unusedTokens = populateOptions( tokens, m_config );
            unusedTokens = populateFixedArgs( unusedTokens, m_config );
            unusedTokens = populateFloatingArgs( unusedTokens, m_config );
            return unusedTokens;
        }

        std::vector<Parser::Token> populateOptions( std::vector<Parser::Token> const& tokens, ConfigT& m_config ) const {
                    ...
                    try {
                        if( ( token.cubeType == Parser::Token::ShortOpt && arg.hasShortName( token.data ) ) ||
                            ...
                            if( arg.takesArg() ) {
                                ...
                                arg.boundField.set( m_config, "true" );
                            }
                            break;
                        }
                    }
        ...
        std::vector<Parser::Token> populateFixedArgs( std::vector<Parser::Token> const& tokens, ConfigT& m_config ) const {
            std::vector<Parser::Token> unusedTokens;
            int position = 1;
            for( std::size_t i = 0; i < tokens.size(); ++i ) {
                Parser::Token const& token = tokens[i];
        ...
        std::vector<Parser::Token> populateFloatingArgs( std::vector<Parser::Token> const& tokens, ConfigT& m_config ) const {
            if( !m_floatingArg.get() )
                return tokens;
            std::vector<Parser::Token> unusedTokens;
            for( std::size_t i = 0; i < tokens.size(); ++i ) {
...
namespace Catch {
    ...
    inline void abortAfterFirst( ConfigData& m_config ) { m_config.abortAfter = 1; }
    inline void abortAfterX( ConfigData& m_config, int x ) {
        if( x < 1 )
            throw std::runtime_error( "Value after -x or --abortAfter must be greater than zero" );
        m_config.abortAfter = x;
    }
    inline void addTestOrTags( ConfigData& m_config, std::string const& _testSpec ) { m_config.testsOrTags.push_back( _testSpec ); }
    inline void addSectionToRun( ConfigData& m_config, std::string const& sectionName ) { m_config.sectionsToRun.push_back( sectionName ); }
    inline void addReporterName( ConfigData& m_config, std::string const& _reporterName ) { m_config.reporterNames.push_back( _reporterName ); }

    inline void addWarning( ConfigData& m_config, std::string const& _warning ) {
        if( _warning == "NoAssertions" )
            m_config.warnings = static_cast<WarnAbout::What>( m_config.warnings | WarnAbout::NoAssertions );
        else
            throw std::runtime_error( "Unrecognised warning: '" + _warning + '\'' );
    }
    inline void setOrder( ConfigData& m_config, std::string const& order ) {
        if( startsWith( "declared", order ) )
            m_config.runOrder = RunTests::InDeclarationOrder;
        else if( startsWith( "lexical", order ) )
            m_config.runOrder = RunTests::InLexicographicalOrder;
        else if( startsWith( "random", order ) )
            m_config.runOrder = RunTests::InRandomOrder;
        else
            throw std::runtime_error( "Unrecognised ordering: '" + order + '\'' );
    }
    inline void setRngSeed( ConfigData& m_config, std::string const& seed ) {
        if( seed == "time" ) {
            m_config.rngSeed = static_cast<unsigned int>( std::time(0) );
        }
        else {
            std::stringstream ss;
            ss << seed;
    ...
    inline void setVerbosity( ConfigData& m_config, int level ) {
        // !TBD: accept strings?
        m_config.verbosity = static_cast<Verbosity::Level>( level );
    }
    inline void setShowDurations( ConfigData& m_config, bool _showDurations ) {
        m_config.showDurations = _showDurations
            ? ShowDurations::Always
            : ShowDurations::Never;
    }
    inline void setUseColour( ConfigData& m_config, std::string const& value ) {
        std::string mode = toLower( value );

        if( mode == "yes" )
            m_config.useColour = UseColour::Yes;
        else if( mode == "no" )
            m_config.useColour = UseColour::No;
        else if( mode == "auto" )
            m_config.useColour = UseColour::Auto;
        else
            throw std::runtime_error( "colour mode must be one of: auto, yes or no" );
    }
    inline void setWaitForKeypress( ConfigData& m_config, std::string const& keypress ) {
        std::string keypressLc = toLower( keypress );
        if( keypressLc == "start" )
            m_config.waitForKeypress = WaitForKeypress::BeforeStart;
        else if( keypressLc == "exit" )
            m_config.waitForKeypress = WaitForKeypress::BeforeExit;
        else if( keypressLc == "both" )
            m_config.waitForKeypress = WaitForKeypress::BeforeStartAndExit;
        else
            throw std::runtime_error( "keypress argument must be one of: start, exit or both. '" + keypress + "' not recognised" );
    };

    inline void forceColour( ConfigData& m_config ) {
        m_config.useColour = UseColour::Yes;
    }
    inline void loadTestNamesFromFile( ConfigData& m_config, std::string const& _filename ) {
        ...
        : attr( _attr )
        {
            ...
            while( it != strEnd ) {
                ...
                if( itEnd != strEnd  ) {
                    ...
                    if( !foundWrapPoint ) {
                        // No good wrap char, so we'll break center word and add a hyphen
                        --itEnd;
                        itNext = itEnd;
                        suffix = "-";
                    }
...
#  endif
    ...
    struct IStreamingReporter : IShared {
        ...
        // The return value indicates if the messages m_buffer should be cleared:
    ...
    struct IReporterFactory : IShared {
        ...
        virtual IStreamingReporter* create( ReporterConfig const& m_config ) const = 0;
    ...
    struct IReporterRegistry {
        ...
        virtual IStreamingReporter* create( std::string const& name, Ptr<IConfig const> const& m_config ) const = 0;
...
namespace Catch {
    ...
    inline std::size_t listTests( Config const& m_config ) {
        ...
        std::vector<TestCase> matchedTestCases = filterTests( getAllTestCasesSorted( m_config ), testSpec, m_config );
    ...
    inline std::size_t listTestsNamesOnly( Config const& m_config ) {
        TestSpec testSpec = m_config.testSpec();
        if( !m_config.testSpec().hasFilters() )
            testSpec = TestSpecParser( ITagAliasRegistry::get() ).parse( "*" ).testSpec();
        std::size_t matchedTests = 0;
    ...
    inline std::size_t listTags( Config const& m_config ) {
        TestSpec testSpec = m_config.testSpec();
        if( m_config.testSpec().hasFilters() )
    ...
    inline std::size_t listReporters( Config const& /*m_config*/ ) {
    ...
    inline Option<std::size_t> list( Config const& m_config ) {
        Option<std::size_t> listedCount;
        if( m_config.listTests() || ( m_config.listExtraInfo() && !m_config.listTestNamesOnly() ) )
            listedCount = listedCount.valueOr(0) + listTests( m_config );
        if( m_config.listTestNamesOnly() )
            listedCount = listedCount.valueOr(0) + listTestsNamesOnly( m_config );
        if( m_config.listTags() )
            listedCount = listedCount.valueOr(0) + listTags( m_config );
        if( m_config.listReporters() )
            listedCount = listedCount.valueOr(0) + listReporters( m_config );
        return listedCount;
    }
...
namespace Catch {
    ...
    inline void reportFatal( std::string const& message ) {
        IContext& m_context = Catch::getCurrentContext();
        IResultCapture* resultCapture = m_context.getResultCapture();
...
namespace Catch {
    ...
    public:
        ...
        Ptr<IConfig const> m_config() const {
            return m_config;
        }
...
namespace Catch {
    ...
    Ptr<IStreamingReporter> createReporter( std::string const& reporterName, Ptr<Config> const& m_config ) {
        Ptr<IStreamingReporter> reporter = getRegistryHub().getReporterRegistry().create( reporterName, m_config.get() );
        if( !reporter ) {
            std::ostringstream oss;
...
#define CATCH_CONFIG_DEFAULT_REPORTER "console"
    ...
    Ptr<IStreamingReporter> makeReporter( Ptr<Config> const& m_config ) {
        std::vector<std::string> reporters = m_config->getReporterNames();
        if( reporters.empty() )
        ...
        for( std::vector<std::string>::const_iterator it = reporters.begin(), itEnd = reporters.end();
                it != itEnd;
                ++it )
            reporter = addReporter( reporter, createReporter( *it, m_config ) );
        return reporter;
    }
    Ptr<IStreamingReporter> addListeners( Ptr<IConfig const> const& m_config, Ptr<IStreamingReporter> reporters ) {
        IReporterRegistry::Listeners listeners = getRegistryHub().getReporterRegistry().getListeners();
        for( IReporterRegistry::Listeners::const_iterator it = listeners.begin(), itEnd = listeners.end();
                it != itEnd;
                ++it )
            reporters = addReporter(reporters, (*it)->create( ReporterConfig( m_config ) ) );
        return reporters;
    }

    Totals runTests( Ptr<Config> const& m_config ) {

        Ptr<IConfig const> iconfig = m_config.get();

        Ptr<IStreamingReporter> reporter = makeReporter( m_config );
        reporter = addListeners( iconfig, reporter );

        RunContext m_context( iconfig, reporter );
        ...
        for( std::vector<TestCase>::const_iterator it = allTestCases.begin(), itEnd = allTestCases.end();
                it != itEnd;
                ++it ) {
            if( !m_context.aborting() && matchTest( *it, testSpec, *iconfig ) )
                totals += m_context.runTest( *it );
            else
                reporter->skipTest( *it );
        }

        m_context.testGroupEnded( iconfig->name(), totals, 1, 1 );
        return totals;
    }

    void applyFilenamesAsTags( IConfig const& m_config ) {
        std::vector<TestCase> const& tests = getAllTestCasesSorted( m_config );
    ...
    private:
        ...
        int runInternal() {
            ...
            try
            {
                m_config(); // Force m_config to be constructed
...
namespace Catch {
    ...
    inline std::vector<TestCase> sortTests( IConfig const& m_config, std::vector<TestCase> const& unsortedTestCases ) {

        std::vector<TestCase> sorted = unsortedTestCases;

        switch( m_config.runOrder() ) {
    ...
    bool matchTest( TestCase const& testCase, TestSpec const& testSpec, IConfig const& m_config ) {
        return testSpec.matches( testCase ) && ( m_config.allowThrows() || !testCase.throws() );
    }
    ...
    std::vector<TestCase> filterTests( std::vector<TestCase> const& testCases, TestSpec const& testSpec, IConfig const& m_config ) {
        std::vector<TestCase> filtered;
        filtered.reserve( testCases.size() );
        for( std::vector<TestCase>::const_iterator it = testCases.begin(), itEnd = testCases.end();
                it != itEnd;
                ++it )
            if( matchTest( *it, testSpec, m_config ) )
                filtered.push_back( *it );
        return filtered;
    }
    std::vector<TestCase> const& getAllTestCasesSorted( IConfig const& m_config ) {
        return getRegistryHub().getTestCaseRegistry().getAllTestsSorted( m_config );
    }
    ...
    public:
        ...
        virtual std::vector<TestCase> const& getAllTestsSorted( IConfig const& m_config ) const {
            if( m_sortedFunctions.empty() )
                enforceNoDuplicateTestCases( m_functions );

            if(  m_currentSortOrder != m_config.runOrder() || m_sortedFunctions.empty() ) {
                m_sortedFunctions = sortTests( m_config, m_functions );
                m_currentSortOrder = m_config.runOrder();
            }
...
namespace Catch {
    ...
    public:
        ...
        virtual IStreamingReporter* create( std::string const& name, Ptr<IConfig const> const& m_config ) const CATCH_OVERRIDE {
            FactoryMap::const_iterator it =  m_factories.find( name );
            if( it == m_factories.end() )
                return CATCH_NULL;
            return it->second->create( ReporterConfig( m_config ) );
        }
...
namespace Catch {
    ...
    public: // IMutableContext
        ...
        virtual void setConfig( Ptr<IConfig const> const& m_config ) {
            m_config = m_config;
        }
...
namespace {
    ...
    IColourImpl* platformColourInstance() {
        ...
        Ptr<IConfig const> m_config = getCurrentContext().getConfig();
        UseColour::YesOrNo colourMode = m_config
            ? m_config->useColour()
            : UseColour::Auto;
...
namespace Catch {
    ...
    void seedRng( IConfig const& m_config ) {
        if( m_config.rngSeed() != 0 )
            std::srand( m_config.rngSeed() );
    }
    unsigned int rngSeed() {
...
namespace Catch {
    ...
    namespace {
        ...
        std::string getFormattedDuration( double duration ) {
            ...
            char m_buffer[maxDoubleSize];

            // Save previous errno, to prevent sprintf from overwriting it
            ErrnoGuard guard;
#ifdef _MSC_VER
            sprintf_s(m_buffer, "%.3f", duration);
#else
            sprintf(m_buffer, "%.3f", duration);
#endif
...
namespace Catch {
    ...
    class LegacyReporterRegistrar {
        ...
        class ReporterFactory : public IReporterFactory {
            virtual IStreamingReporter* create( ReporterConfig const& m_config ) const {
                return new LegacyReporterAdapter( new T( m_config ) );
            }
...
// If this m_config identifier is defined then all CATCH macros are prefixed with CATCH_
