...
class ProtoSignal<R(Args...), Collector> : private CollectorInvocation<
 ...
 public:
  ...
  /// Operator to removeComponent a signal handler through it connection ID, returns if a
  /// handler was removed.
 ...
 * The callbacks of a signal are invoked with the post() method and arguments
 * according to the signature.
 * The result returned by post() depends on the signal collector class. By
 * default, the result of
 * the last callback is returned from post(). Collectors can be implemented to
 * accumulate callback
 ...
 * removed and
 * added during a signal emission and recursive post() calls are also safe.
